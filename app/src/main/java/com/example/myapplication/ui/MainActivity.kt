...
import com.example.myapp.polish.PolishString
import com.example.myapp.polish.calculatePolishString
import com.example.myapplication.CodeBlockVar
...
var mapp = mutableMapOf<String, Int>()
...
//                blocksList.add(CodeBlock(id=CodeBlock.ID))
        ...
        //    }
            for (block in blocksList) {
                block.value = calculatePolishString(PolishString(block.value).expression, mapp).toString()
                mapp[block.name] = block.value.toInt()
                if(block.name == "res") {
                    Log.d("Block print", "$block")
                    break
                }
